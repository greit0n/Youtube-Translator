...
Translation engines:
- **`ollama`** (current default in code, default model `gemma2:9b`): Whisper `task="transcribe"` (faithful source language) → local Ollama chat model translates, preserving profanity/slang/names. The `glossary` terms are injected into the prompt to keep translations consistent. Falls back to `whisper` automatically if Ollama is unreachable. This is now the default because Whisper's `translate` task censors/softens profanity and drops slang. Recommended models to pull: `gemma2:9b` (default) and `aya-expanse:8b` — both strong multilingual models that avoid the `qwen2.5:7b` Chinese-character leak (the reason `qwen2.5:7b` is no longer the default model).
- **`whisper`** (fallback): Whisper `task="translate"` → English directly. Faster, single-step; used automatically when Ollama is unreachable.

Settings (`engine`, `model`, `language`, `preBuffer`, `quality`, `cleanAudio`, `diarize`, `enrolledOnly`, `glossary`) live in `chrome.storage.sync` (defaults in `extension/background.js`), are sent in the WS start message, and changing any of them triggers a full session reinit (reconnect) in `content.js`. The `glossary` is a multiline editor (one entry per line, `term` or `term = preferred`); terms also bias Whisper recognition as hotwords.

## WS protocol (shared contract — keep `content.js` and `server.py` in lockstep)

- **Client → server, first message:** `{videoId, startTime, language(null=auto), engine, model, preBuffer, quality, cleanAudio, diarize, enrolledOnly, hotwords, glossary}` — `hotwords` derived from the glossary; `glossary` is an array of `{term, preferred}`; `enrolledOnly` (bool) keeps only the enrolled "your voice" speaker (drops game audio + other speakers), implies diarization, and needs an enroll clip in `helper/enroll/` + an HF token — degrades gracefully to all-speakers with a status warning if either is missing.
